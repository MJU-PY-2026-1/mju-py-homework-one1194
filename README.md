...
## 2. 시나리오 (5~10줄)
* 이 프로그램은 사용자가 축구팀의 감독이 되어 선수단을 관리하고 경기를 치르는 '구단 운영 시뮬레이션'입니다. 사용자는 자본금(예: 1,000만 원)을 가지고 게임을 시작하며, 상점에서 실존 선수를 모티브로 한 선수(예: 손흥민 - 공격 90, 수비 40)를 영입하거나 방출할 수 있습니다. 추가로 내가 직접 선수가 되어 뛰어볼 수도 있습니다. 직접 타이핑하여 선수를 생성할 수도 있습니다.(예: 제갈원 - 공격 85, 드리블 78) 선수 데이터는 파이썬의 리스트를 활용해 '나의 스쿼드'로 관리됩니다. 정규 시즌이 시작되기 전 이적 시장을 통해 선수들을 영입하거나 방출할 수도 있습니다. 
...
1. 내 구단 스쿼드 및 재정 조회
2. 선수 영입 및 방출 (이적시장)
3. 선수단 훈련 (스탯 상승, 체력 감소)
4. 경기 진행 (경기 결과 및 보상 시뮬레이션)
5. 선수 부상/체력 회복소 (휴식 기능)
6. 게임 종료
...
### **🤖 AI 파트너십 과정**
 1. **내용 1 : 길드 매니저 컨셉 도출**
    ...
    * **적용 내용:** AI가 선수들의 초기 데이터는 코드 안에 미리 리스트 형태로 만드는 것도 좋지만, 프로그램 실행 후 사용자에 따라서 직접 타이핑하여 새로운 선수를 생성(create)하는 방식도 좋을 것 같다고 추천해주었음.
