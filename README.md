[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/MiVO5zfB)
# 🐍 [2026-01] 나만의 파이썬 소프트웨어 개발 프로젝트

## 1. 시나리오 제목
* 축구 감독이 되어 선수들을 관리하고 정규 시즌을 치러 경기를 수행하는 시스템
<br>


## 2. 시나리오 (5~10줄)
* 이 프로그램은 사용자가 축구팀의 감독이 되어 선수단을 관리하고 경기를 치르는 '구단 운영 시뮬레이션'입니다. 사용자는 자본금(예: 1,000만 원)을 가지고 게임을 시작하며, 상점에서 실존 선수를 모티브로 한 선수(예: 손흥민 - 공격 90, 속도 93, 수비 50)를 영입하거나 방출할 수 있습니다. 추가로 직접 타이핑하여 선수를 생성할 수도 있어 내가 직접 선수가 되어 뛰어볼 수도 있습니다. (예: 제갈원 - 공격 85, 드리블 78) 선수 데이터는 파이썬의 리스트를 활용해 '나의 스쿼드'로 관리됩니다. 정규 시즌이 시작되기 전 이적 시장을 통해 선수들을 영입하거나 방출할 수도 있습니다. 
스쿼드가 완성되면 '경기 치르기' 메뉴를 통해 상대 팀과 대결합니다. 시스템은 출전 선수들의 총합 공격력과 수비력을 계산하고, 랜덤 변수를 더해 경기 결과를 출력합니다. 승리 시 상금과 선수 경험치를 얻어 능력치가 상승(S, A, B 등급 부여)하지만, 체력이 깎이는 복합적인 조건 로직이 포함됩니다. 최종적으로 메뉴를 통해 선수 영입, 스쿼드 관리, 경기 진행을 자유롭게 오가며 최고의 구단을 만드는 것이 목표입니다.

<br>

## 3. 예상 기능 및 메뉴 (최소 5개)
1. 선수 능력치 확인 시스템 
2. 내 구단 스쿼드 관리 및 재정 조회
3. 선수 영입 및 방출 (이적시장)
4. 선수단 훈련 (스탯 상승, 체력 감소)
5. 경기 진행 (경기 결과 및 보상 시뮬레이션)
6. 선수 부상/체력 회복소 (휴식 기능)
7. 게임 종료

<br>

# 🚀 [버전별 개발 일지 & AI 협업 기록]

## 🟦 [1차 과제: V1.0] 시나리오 기획
    
### **🤖 AI 파트너십 과정**
 1. **내용 1 : 길드 매니저 컨셉 도출**
    * **프롬프트 요약:** 파이썬으로 football manager 같은 게임 프로그램을 만들고 싶어. 내가 감독이 되어서 선수들을 관리하고 영입 시장을 진행하며 내 취향대로 선수들을 꾸리는거야, 그리고 경기까지 간단하게 진행해서 구단까지 운영하는 느낌으로 진행하는 프로그램인거지. 파이썬 입문자인 내가 코딩할 수 있을까? 난이도는 어떤 것 같애? 추가로 더 들어갔으면 하는 내용이 있으면 추천해줘
    * **적용 내용:** 기본 뼈대를 우선적으로 만들고 살을 붙인다면 훌룡할 것. AI가 선수들의 초기 데이터는 코드 안에 미리 리스트 형태로 만드는 것도 좋지만, 프로그램 실행 후 사용자에 따라서 직접 타이핑하여 새로운 선수를 생성(create)하는 방식도 좋을 것 같다고 추천해주었음.
      
 2. **내용 2 : 로드맵 도출**
    * **프롬프트 요약:** 축구 선수 데이터 구조 및 자료형 모델링, 신규 선수 생성, 경기 시뮬레이션 논의
    * **적용 내용:** 이름과 포지션은 문자열(str), 공격력/수비력/체력은 정수형(int), 이적료나 평균 평점은 실수형(float)으로 명확히 분리하고, 이를 리스트(List)로 묶어 전체 스쿼드를 관리함으로써 데이터 접근성과 확장성을 높임. input() 함수를 통해 감독(사용자)으로부터 직접 이름과 능력치를 입력받아 새로운 선수 딕셔너리를 생성하고 스쿼드 리스트에 추가(append)하는 기능을 구현함. if-elif-else을 통해 승/무/패 결과를 도출하는 현실적인 경기 시뮬레이션 로직을 구현함.
    
### **📁 증빙 자료:**
  * [1차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)

<br>

## 🟩 [2차 과제: V1.0] 입출력 + 리스트 + 조건문 - 향후 작성 예정
### **✨2차 과제 내용:**
  * 이번 과제는 용사 등록 및 정밀 진단 시스템 구현으로, 사용자로부터 데이터를 입력받아 리스트에 저장하고, 가중치 연산을 통해 결과값을 도출하는 시스템을 구축하는 것입니다. 특히 `if-elif` 조건문과 `and/or` 논리 연산자를 활용하여 상황에 맞는 등급과 특별 칭호를 부여하는 지능형 로직을 완성해야 합니다.
  * input()으로 용사의 이름, 체력, 마력, 공격력을 입력받아 리스트에 저장.
  * 가중치 공식을 적용하여 종합 전투력 산출: (체력 * 0.7) + (마력 * 0.5) + (공격력 * 1.8).
  * if-elif-else를 사용하여 S등급부터 F등급까지 5단계 등급 판정 로직 추가.
  * and 연산자를 활용하여 '전설의 대마법사' 등 특별 칭호 부여 기능 구현.
    
### **🤖 AI 파트너십 과정**
(예시) 
1. **내용 1: 길드 매니저 컨셉 및 로직 도출**
    * **프롬프트 요약:** "내가 길드 매니저가 되어 용사를 관리하는 프로그램을 만들 거야. 1학년 수준에서 리스트와 조건문을 활용해 등급을 매기는 재미있는 로직을 짜줘."
    * **적용 내용:** AI가 제안한 '가중치 전투력 산출' 방식과 'S등급이면서 마력이 높으면 대마법사 칭호 부여'라는 복합 조건 아이디어를 실제 코드에 반영함.

2. **내용 2: 효율적인 리스트 활용법 자문**
    * **프롬프트 요약:** "파이썬 기초 문법 단계(리스트 → 조건문 → 반복문 → 함수/딕셔너리)에 맞춰, '용사 길드 관리자' 컨셉의 프로그램이 어떻게 발전할 수 있을지 4단계 기능 확장 로드맵을 제안해줘."
    * **적용 내용:** 단순 입출력을 넘어, **'기초 정보 입력 및 리스트 저장(1차) → 전투 등급 판독기 구현(2차) → 무한 메뉴 시스템 구축(3차) → 딕셔너리를 활용한 다중 캐릭터 관리(4차)'**로 이어지는 논리적인 확장 로드맵을 확립함.
    
### **🛠️ Troubleshooting & 기술 회고:**
(예시) 
1. **문제 1: 숫자를 입력했는데 계산이 안 되는 현상 (TypeError)**
    * **원인:** `input()`으로 받은 데이터는 기본적으로 '글자'라서 숫자와 곱셈 연산이 불가능했음.
    * **해결:** `int()`와 `float()` 함수를 사용하여 입력값을 숫자로 바꾸는 '형변환' 마법을 부려 해결함.
2. **문제 2: 조건문 뒤에 자꾸 생기는 빨간 줄 (SyntaxError)**
    * **원인:** `if`와 `elif` 조건식 끝에 마법의 기호인 `:`(콜론)을 자꾸 빠뜨려서 발생함.
    * **해결:** 파이썬의 모든 조건문과 반복문 끝에는 항상 땡땡(`:`)이 들어가야 한다는 규칙을 학습하고 수정함.
     
### **📁 증빙 자료:**
  * [2차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)
  * [2차과제_실행결과.jpg]
<br>

## 🟨 [3차 과제: V3.0] 무한 루프와 메뉴 시스템 (반복문) - 향후 작성 예정
### **✨3차 과제 업데이트 내용:**
  * 내용.
    
### **🤖 AI 파트너십 과정**
 1. **내용 1**
    * **프롬프트 요약:**  ... 
    * **적용 내용:** ....
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** ...
     * **원인:** ...
     * **해결:** ..
     
### **📁 증빙 자료:**
  * [3차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)
  * [3차과제_실행결과.jpg]
<br>

### 🟥 [4차 과제: V4.0] 모듈화 및 데이터 확장 (배열과 함수) - 🌟최종 완성 -- 향후 작성 예정
### **✨4차 과제 업데이트 내용:**
  * 내용.
    
### **🤖 AI 파트너십 과정**
 1. **내용 1**
    * **프롬프트 요약:**  ... 
    * **적용 내용:** ....
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** ...
     * **원인:** ...
     * **해결:** ..
     
### **📁 증빙 자료:**
  * [4차_AI협업캡처.pdf 첨부 완료] (첨부 후 링크)
  * [4차과제_실행결과.jpg]
<br>
